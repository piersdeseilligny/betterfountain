...
export
  ...
  protected onResize(msg: Widget.ResizeMessage): void {
    if (this.parent!.isVisible) {
      this._update(msg.width, msg.height, false);
    }
  }
  ...
  private _fit(): void {
    ...
    if (this._root) {
      let limits = this._root.fit(this._spacing, this._items, this._topTabsContainer);
   ...
   */
  private _update(offsetWidth: number, offsetHeight: number, updateLayout: boolean): void {
    ...
    let y = this._box.paddingLeft-toptabChange;
    let width = offsetWidth - this._box.horizontalSum;
    let height = offsetHeight - this._box.verticalSum + toptabChange;


    // Update the geometry of the layout tree.
    this._root.update(x, y, width, height, this._spacing, this._items, updateLayout);
  }
...
namespace Private {
  ...
  export
     ...
     * Fit the layout tree.
     */
    fit(spacing: number, items: ItemMap, toptabsContainer: SplitPanel): ElementExt.ISizeLimits {
  ...
  function isTabLayoutNode(obj: any): obj is TabLayoutNode {
    return obj.toptabsContainer !== undefined;
  }
  ...
  export
    ...
    fit(spacing: number, items: ItemMap, toptabsContainer: SplitPanel): ElementExt.ISizeLimits {
      ...
      for (let i = 0, n = this.children.length; i < n; ++i) {
        let limits = this.children[i].fit(spacing, items, toptabsContainer);
        if (horizontal) {
     ...
     * Update the layout tree.
     */
    update(left: number, top: number, width: number, height: number, spacing: number, items: ItemMap, updateLayout: boolean): void {
      ...
      for (let i = 0, n = this.children.length; i < n; ++i) {
        ...
        if (horizontal) {
          child.update(left, top, size, height, spacing, items, updateLayout);
          ...
          child.update(left, top, width, size, spacing, items, updateLayout);
          top += size;
          handleStyle.top = `${top}px`;
          handleStyle.left = `${left}px`;
