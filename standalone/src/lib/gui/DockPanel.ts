...
import { TabBar } from './TabBar';
   ...
   */
  export
  class DockPanelAlt extends Widget {
    /**
    ...
    constructor(options: DockPanelAlt.IOptions = {}) {
      ...
      this._edgesEnabled = options.edgesEnabled || Private.DEFAULT_EDGESENABLED;
      ...
      let renderer: DockPanelAlt.IRenderer = {
     ...
     */
    readonly overlay: DockPanelAlt.IOverlay;
  
    /**
     * The renderer used by the dock panel.
     */
    get renderer(): DockPanelAlt.IRenderer {
     ...
     */
    set mode(value: DockPanelAlt.Mode) {
     ...
     */
    saveLayout(): DockPanelAlt.ILayoutConfig {
     ...
     */
    restoreLayout(config: DockPanelAlt.ILayoutConfig): void {
      // Reset the mode.
     ...
     */
    addWidget(widget: Widget, options: DockPanelAlt.IAddOptions = {}): void {
    ...
    private _evtDrop(event: IDragEvent): void {
      ...
      (this.layout as DockLayout).onUpdateRequest(undefined); //update the layout again
    }
    ...
    private _onTabDetachRequested(sender: TabBar<Widget>, args: TabBar.ITabDetachRequestedArgs<Widget>): void {
      ...
      console.log("tab detach request");
    ...
    private _edges: DockPanelAlt.IEdges;
    private _edgesEnabled: DockPanelAlt.IEdgesEnabled;
  ...
  namespace DockPanelAlt {
    ...
    interface IOptions {
      ...
      /**
       * Which edges are enabled
       * 
       * The default is true for all of them
       */
      edgesEnabled?: IEdgesEnabled;
    ...
    export interface IEdgesEnabled{
      top:boolean;
      right:boolean;
      bottom:boolean;
      left:boolean;
  ...
  namespace Private {
    ...
    export const DEFAULT_EDGESENABLED = {
      top:true,
      right:true,
      bottom:true,
      left:true
     ...
     */
    export
    function createSingleDocumentConfig(panel: DockPanelAlt): DockPanelAlt.ILayoutConfig {
    ...
    function findDropTarget(
      ...
      clientX: number,
      clientY: number,
      edges: DockPanelAlt.IEdges,
      edgesEnabled: DockPanelAlt.IEdgesEnabled
    ): IDropTarget {
      ...
      if (panel.mode === 'multiple-document') {
        ...
        if(!edgesEnabled.bottom) pb = Infinity;
        if(!edgesEnabled.right) pr = Infinity;
        if(!edgesEnabled.left) pl = Infinity;
        if(!edgesEnabled.top) pt = Infinity;
      ...
      if(!edgesEnabled.bottom) ab = Infinity;
      if(!edgesEnabled.right) ar = Infinity;
      if(!edgesEnabled.left) al = Infinity;
      if(!edgesEnabled.top) at = Infinity;
