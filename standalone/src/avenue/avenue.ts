...
export const regexes: { [index: string]: RegExp } = {
    emptyline: /^$/gm
}
...
export class AvenueParser {
    ...
    public parseChanges(change: monaco.editor.IModelContentChange[]) {
        return;
    ...
    private parseRange(start: number, end: number, text:string) {
    ...
    public parseDocument(document: string) {
        this.parseRange(0, document.length, document);
        console.log(regex);
    }
}
