...
function updateWebView(titlepage: string, script: string) {
	...
	previewpanel.webview.html = "<html><head><style>" + css + "</style></head><body id='fountain-js'><section id='workspace' style='display:block;'><div id='script' class='us-letter dpi100'><div id='titlepage' class='page title-page'>" + titlepage + "</div><div id='mainpage' class='"+ pageClasses +"'>" + script + "</div></div></section>" +
		`<script>
	...
	}else if(event.data.command == 'updatePageClasses'){
		document.getElementById('mainpage').className = event.data.content;
	}
	});
	</script></body></html>`;
